...
extension UINavigationBar {
    ...
    private var hairlineView: UIImageView? {
        return recursiveSubviews.compactMap({ $0 as? UIImageView }).filter({ $0.bounds.size.width == self.bounds.size.width }).first(where: { $0.bounds.size.height <= 2 })
    }
    
    open override func tintColorDidChange() {
        super.tintColorDidChange()
        
        self.items?.compactMap({ (item) -> [UIView]? in
            let right = item.rightBarButtonItems?.compactMap({$0.customView}) ?? []
            let left = item.leftBarButtonItems?.compactMap({$0.customView}) ?? []
